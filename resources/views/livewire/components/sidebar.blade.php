<div wire:key="search-component" x-data="{
    shrink: $wire.entangle('shrink'),
    drawer: $wire.entangle('drawer'),
    showNotifications: $wire.entangle('showNotifications')
}" :class="{'w-72': !shrink}" class="menu p-3 w-20 h-full border-r border-stone-800 text-base-content z-10">

    <div class="py-3 px-2">
        <div x-show="shrink || drawer" class="w-full pl-2">
        ...
        <div x-cloak x-show="!(shrink ||drawer)">
            <a href="">
                <svg aria-label="Instagram" class="h-8 w-28" color="#fafafa" fill="#fafafa" height="29" role="img"
    ...
    <ul class="space-y-4 mt-2">
        <li>
            <a wire:navigate href="{{route('home')}}" class="flex items-center gap-5">
                <span>
                    @if (request()->routeIs('home'))
                    <svg aria-label="Home" class="size-6" color="#fafafa" fill="#fafafa" height="24" role="img"
                    ...
                    @else
                    <x-lucide-house class="size-6" />
                    @endif
                </span>
                <h4 x-cloak x-show="!(shrink||drawer)"
                    class="text-lg {{request()->routeIs('Home') ? 'font-extrabold' : 'font-medium'}}">Home</h4>
            </a>
        </li>

        <li>
            <div class="flex items-center gap-5" x-on:click="$wire.toggleDrawer">
                <span>
                    <svg aria-label="Search" class="size-6" color="#fafafa" fill="#fafafa" height="24" role="img"
                        viewBox="0 0 24 24" width="24">
        ...
        <li>
            <a wire:navigate href="{{route('explore')}}" class="flex items-center gap-5">
                <span>
                    ...
                    <svg aria-label="Explore" class="size-6" color="#fafafa" fill="#fafafa" height="24" role="img"
                        viewBox="0 0 24 24" width="24">
                ...
                <h4 x-cloak x-show="!(shrink||drawer)"
                    class="{{request()->routeIs('explore') ? 'font-extrabold' : 'font-medium'}} text-lg">Explore
                </h4>
            </a>
        </li>
        ...
        <li>
            <a wire:navigate href="{{route('reels')}}" class="flex items-center gap-5">
                ...
                <h4 x-cloak x-show="!(shrink||drawer)" class=" text-lg">Reels</h4>
            </a>
        </li>
        ...
        <li>
            <a href="" class="flex items-center gap-5">
                <span>
                    <svg color="#FFF" fill="#FFF" height="24" viewBox="0 0 24 24" width="24">
                        <path
        ...
        <li>
            <a href="" class="flex items-center gap-5">
                <span>
                    <svg class="size-6" color="#fafafa" fill="#fafafa" height="24" viewBox="0 0 24 24" width="24">
                        <path
        ...
        <li>
            <div onclick="Livewire.dispatch('openModal', { component: 'post.create' })" class="flex items-center gap-5">
                <svg color="#fafafa" fill="#fafafa" height="24" viewBox="0 0 24 24" width="24" class="size-6">
                    <path
        ...
        <li>
            <a wire:navigate href="{{route('profile.home', auth()->user()->username)}}" class="flex items-center gap-5">
                <x-avatar src="https://github.com/shadcn.png" class=" w-7 h-7 shrink-0" />
                <h4 x-cloak x-show="!(shrink||drawer)" class=" text-lg">Profile</h4>
            </a>
        </li>
        @endauth
    </ul>
    ...
    <footer class="sticky bottom-0 mt-auto w-full px-3 z-50">
        <div class="dropdown dropdown-top">
            <div tabindex="0"
                class="flex cursor-pointer mt-[75px] text-xl rounded-3xl py-2 hover:bg-gray-50 hover:bg-opacity-10">
                <div class="pl-3 ">
                    <div class="h-[2px] w-5 bg-white my-[6px] mr-4"></div>
                    <div class="h-[2px] w-5 bg-white my-[6px] mr-4"></div>
                    <div class="h-[2px] w-5 bg-white my-[6px] mr-4"></div>
                </div>
                <span>
                    <p class="text-white text-base mt-1">More</p>
                </span>
            </div>

            <ul tabindex="0"
                class="dropdown-content z-[1] menu p-2 space-y-3 shadow bg-base-100 border border-gray-800 rounded-md w-60">
                <li>
                ...
                <li>
                    <!-- Authentication -->
                    <button wire:click="logout" class="py-2">
                        Logout
                    </button>
                </li>
            </ul>
    ...
    <div x-data="{showSearch: $wire.entangle('showSearch')}" x-show="drawer" x-cloak x-transition.origin.left
        @click.outside="$wire.closeDrawer()"
        class="fixed inset-y-0 left-[70px] w-96 px-4 overflow-y-scroll overflow-x-hidden bg-black z-[9999] border-r border-gray-800">
        <template x-if="showSearch">
            <div class="h-full">
                <header class="sticky top-0 w-full z-10 bg-black py-2 px-4">
                    <h5 class="text-2xl font-bold my-4">Search</h5>
                    <input wire:model.live.debounce.300ms="search" type="search" placeholder="Search"
                        class="flex w-full h-10 px-3 py-2 text-sm bg-gray-900 border rounded-md border-neutral-800/70 ring-offset-background placeholder:text-neutral-200 focus:border-neutral-700/30 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-900/60 disabled:cursor-not-allowed disabled:opacity-50">
                </header>
                    ...
                    <ul class="space-y-2 overflow-x-hidden">
                        ...
                        <li wire:key="{{ $user->id }}">
                            <a href="{{ route('profile.home', $user->username) }}"
                                class="flex gap-2 items-center truncate">
                                <x-avatar wire:ignore class="size-9 mb-auto" />
                                <div class="flex flex-col">
                                    <span class="font-bold text-sm">{{ $user->username }}</span>
                                    <span class="font-normal text-xs truncate">{{ $user->description }}</span>
                                </div>
                            </a>
                        </li>
                        @endforeach
